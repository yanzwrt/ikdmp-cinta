...
// Endpoint refresh ONU
router.post('/genieacs/refresh-onu', adminAuth, async (req, res) => {
  try {
    ...
    try {
      await axios.post(`${genieacsUrl}/devices/${encodedDeviceId}/tasks?connection_request`, payload, axiosConfig);
    } catch (firstErr) {
      await axios.post(`${genieacsUrl}/devices/${encodedDeviceId}/tasks`, payload, axiosConfig);
    }

    res.json({ success: true, message: 'Perintah refresh berhasil dikirim ke device' });
  } catch (err) {
    const raw = err.response?.data;
    const detail = typeof raw === 'string' ? raw : (raw?.message || JSON.stringify(raw || {}));
    console.error('Error refresh ONU:', detail || err.message);
    res.status(500).json({
      success: false,
      message: 'Gagal mengirim perintah refresh: ' + (detail || err.message)
    });
  }
});
...
router.get('/api/statistics', adminAuth, async (req, res) => {
  try {
    ...
    const now = Date.now();
    const onlineDevices = devicesRaw.filter(dev => dev._lastInform && (now - new Date(dev._lastInform).getTime()) < 3600*1000).length;
