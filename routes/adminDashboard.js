...
const { getAdminInboxConversations } = require('../config/roleChatManager');
...
async function buildDashboardNotifications({ genieacsOffline = 0, mikrotikOffline = 0 } = {}) {
  ...
  try {
    const genericChats = await getAdminInboxConversations();
    genericChats
      .filter((item) => Number(item.unreadCount || 0) > 0)
      .slice(0, 8)
      .forEach((item) => {
        dashboardNotifications.unshift({
          id: `role-chat-${item.role}-${item.userId}`,
          type: 'primary',
          icon: 'bi-chat-dots-fill',
          title: `Chat ${item.role}`,
          message: `${item.participant?.name || item.role} mengirim ${item.unreadCount} pesan yang belum dibaca.`,
          meta: 'Buka percakapan',
          link: `/admin/chats/${item.role}/${item.userId}`
        });
      });
  } catch (chatError) {
    console.warn('[DASHBOARD] Gagal memuat notifikasi chat role umum:', chatError.message);
  }
