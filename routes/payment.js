            ...
            setImmediate(async () => {
                try {
                    await notifyCustomer({
                        ...
                        message: `Tagihan ${invoice.invoice_number || `INV-${invoice.id}`} berhasil dibayar.`,
                        url: `/customer/billing/invoices/${invoice.id}`,
                        tag: `payment-${invoice.id}`
                    }, customer || {});
                } catch (error) {
                    console.error('Push payment customer gagal:', error.message);
                }
            });
...
router.get('/failed-payments', async (req, res) => {
    try {
        const transactions = await billingManager.getPaymentTransactions();
        const failedOrPending = (transactions || [])
            .filter(tx => ['pending', 'failed', 'expire', 'expired', 'cancel', 'cancelled', 'deny'].includes(String(tx.status || '').toLowerCase()))
            .slice(0, 50);

        res.json({
            success: true,
            payments: failedOrPending
        });
    } catch (error) {
        console.error('Error getting failed payments:', error);
        res.status(500).json({
            success: false,
            message: error.message || 'Failed to load failed payments'
        });
    }
});

router.get('/active-pending-payments', async (req, res) => {
    try {
        const payments = await billingManager.getActivePendingGatewayTransactions();
        res.json({
            success: true,
            payments
        });
    } catch (error) {
        console.error('Error getting active pending payments:', error);
        res.status(500).json({
            success: false,
            message: error.message || 'Failed to load active pending payments'
        });
    }
});
...
router.get('/recent-transactions', async (req, res) => {
    try {
        ...
        const allTransactions = (payments || []).map(payment => ({
            id: payment.id,
        ...
        const currentMonthPaidCount = allTransactions.filter(transaction => {
            const paymentDateRaw = String(transaction.payment_date || '').trim();
            return paymentDateRaw.startsWith(currentMonthKey);
        }).length;
