...
// Ambil data backbone / koneksi antar ODP dari odp_connections
// Ambil data backbone / koneksi antar ODP dari odp_connections
const networkSegments = await new Promise((resolve, reject) => {
    db.all(`
        SELECT 
            oc.id,
            'Backbone ' || o1.name || ' - ' || o2.name as name,
            oc.from_odp_id,
            oc.to_odp_id,
            oc.connection_type,
            oc.status,
            ...
            oc.created_at,
            oc.updated_at,
            o1.name as start_odp_name,
            o1.latitude as start_latitude,
            o1.longitude as start_longitude,
            o2.name as end_odp_name,
            o2.latitude as end_latitude,
            o2.longitude as end_longitude
...
// PUT: Update ODP
const updateTechnicianOdpHandler = async (req, res) => {
    try {
        const { id } = req.params;
        const { name, code, latitude, longitude, address, capacity, notes, status, force_notification } = req.body;

        const allowedStatus = ['active', 'maintenance', 'inactive'];
        const normalizedStatus = allowedStatus.includes(String(status || '').toLowerCase())
        ...
        if (!finalName || !finalCode || !finalLatitude || !finalLongitude) {
            db.close();
            return res.status(400).json({
                success: false,
                message: 'Data ODP belum lengkap untuk diupdate'
            });
        }
        ...
        await new Promise((resolve, reject) => {
            db.run(`
                ...
                WHERE id = ?
            `, [finalName, finalCode, finalLatitude, finalLongitude, finalAddress, finalCapacity, finalNotes, normalizedStatus, id], function(err) {
                if (err) reject(err);
        ...
        const isMaintenanceOn = (normalizedStatus === 'maintenance' && previousStatus !== 'maintenance') ||
            (normalizedStatus === 'maintenance' && forceNotify);
        const isInactiveOn = (normalizedStatus === 'inactive' && previousStatus !== 'inactive') ||
            (normalizedStatus === 'inactive' && forceNotify);
        ...
        if (shouldNotify) {
            try {
                const customers = await new Promise((resolve) => {
                    ...
                    `, [id, id], (err, rows) => {
                        if (err) {
                            logger.error('Error fetching customers for technician ODP notification:', err);
                            resolve([]);
                        } else {
                            resolve(rows || []);
                        }
                ...
                if (customers.length > 0) {
                    const companyName = await getSetting('company_name') || 'IKDMP-CINTA';
                    const odpDisplayName = name || existingODP.name || `ODP ${id}`;
                    ...
                    for (const customer of customers) {
                        ...
                        if (isMaintenanceOn) {
                            message = `Yth. Pelanggan IKDMP-CINTA ${customer.name},\n\n` +
                                `Jaringan internet Anda yang terhubung ke ODP ${odpDisplayName} sedang dalam pemeliharaan/perbaikan.\n` +
                                `Mohon maaf atas ketidaknyamanan ini.\n\n` +
                                `Terima kasih,\n${companyName}`;
                        } else if (isInactiveOn) {
                            message = `Yth. Pelanggan IKDMP-CINTA ${customer.name},\n\n` +
                                `Informasi layanan:\n` +
                                `ODP ${odpDisplayName} saat ini sedang dinonaktifkan sementara.\n` +
                            ...
                            message = `Yth. Pelanggan IKDMP-CINTA ${customer.name},\n\n` +
                                `Kabar Gembira!\n` +
                                `Gangguan/pemeliharaan pada ODP ${odpDisplayName} telah SELESAI.\n` +
                        ...
                        if (sent) {
                            logger.info(`[TECHNICIAN-ODP] Notification sent to ${customer.name} (${customer.phone})`);
                        } else {
                            logger.warn(`[TECHNICIAN-ODP] Notification not sent to ${customer.name} (${customer.phone})`);
                        }
                    }
                } else {
                    logger.info('[TECHNICIAN-ODP] Tidak ada pelanggan aktif yang perlu diberi notifikasi');
                }
            } catch (notifyError) {
                logger.error('Error sending technician ODP maintenance notifications:', notifyError);
            }
        }
...
router.put('/api/odp/:id', technicianAuth, updateTechnicianOdpHandler);
router.put('/cable-network/api/odp/:id', technicianAuth, updateTechnicianOdpHandler);
router.post('/api/odp/:id/status', technicianAuth, updateTechnicianOdpHandler);
router.post('/cable-network/api/odp/:id/status', technicianAuth, updateTechnicianOdpHandler);
// DELETE: Delete ODP
