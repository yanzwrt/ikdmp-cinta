...
function slugifyProfileName(value, fallback = 'user') {
...
const customerProfilePhotoUpload = multer({
    storage: multer.diskStorage({
        destination: function (req, file, cb) {
            const uploadPath = path.join(__dirname, '../public/img/customer-profiles');
        ...
        filename: function (req, file, cb) {
            ...
            const username = req.session?.customer_username && !String(req.session.customer_username).startsWith('temp_')
                ? req.session.customer_username
                : (req.session?.customer_phone || req.session?.phone || 'customer');
            cb(null, `${slugifyProfileName(username, 'customer')}${ext}`);
        }
    }),
...
router.get('/dashboard', ensureCustomerSession, getAppSettings, async (req, res) => {
    try {
        ...
        const activeInvoices = invoices.filter(inv => inv.status === 'unpaid');
        const totalInvoices = activeInvoices.length;
        const paidInvoices = 0;
        const unpaidInvoices = activeInvoices.length;
        ...
        res.render('customer/billing/dashboard', {
            title: 'Dashboard Billing',
            customer,
            invoices: activeInvoices.slice(0, 5), // 5 tagihan aktif terbaru
...
router.get('/invoices', ensureCustomerSession, getAppSettings, async (req, res) => {
    try {
        ...
        invoices = invoices.filter(invoice => invoice.status === 'unpaid');
...
router.get('/profile', ensureCustomerSession, getAppSettings, async (req, res) => {
    try {
        ...
        const selectedPackage = (customer && customer.package_name)
            ? customer.package_name
            : (packages || []).find(p => String(p.id) === String(customer?.package_id))?.name || '-';

        const profileData = {
            name: customer?.name || 'Pelanggan',
            username: customer?.username || customer?.pppoe_username || username || '-',
...
router.post('/profile/photo', ensureCustomerSession, customerProfilePhotoUpload.single('profile_photo'), async (req, res) => {
    try {
        const username = req.session.customer_username;
        const phone = req.session.customer_phone || req.session.phone;
        const uploadDir = path.join(__dirname, '../public/img/customer-profiles');
        if (!req.file) {
        ...
        db.run('UPDATE customers SET profile_photo = ? WHERE id = ?', [req.file.filename, customer.id], function (err) {
            if (err) {
                deleteProfileFileIfExists(uploadDir, req.file.filename);
                logger.error('Error saving customer self profile photo:', err);
                return res.status(500).json({ success: false, message: 'Gagal menyimpan foto profil' });
            }
            if (customer.profile_photo && customer.profile_photo !== req.file.filename) {
                deleteProfileFileIfExists(uploadDir, customer.profile_photo);
            }
            return res.json({
                success: true,
