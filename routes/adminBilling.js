...
const createImageStorage = (subDir) => multer.diskStorage({
    destination: function (req, file, cb) {
        const uploadPath = path.join(__dirname, `../public/img/${subDir}`);
        if (!fs.existsSync(uploadPath)) {
            fs.mkdirSync(uploadPath, { recursive: true });
        }
        cb(null, uploadPath);
...
const uploadCustomerHousePhoto = multer({
    storage: createImageStorage('customer-houses'),
    fileFilter: fileFilter,
    limits: {
...
const uploadCustomerProfilePhoto = multer({
    storage: createImageStorage('customer-profiles'),
    fileFilter: fileFilter,
    limits: {
        fileSize: 3 * 1024 * 1024 // 3MB
    }
    ...
    try {
        ...
        if (overdueInvoices.length > 0) {
            ...
            billingNotifications.push({
                ...
                message: `${overdueInvoices.length} tagihan terlambat. Prioritas terdekat: ${latestOverdue.customer_name || 'Pelanggan'} - ${latestOverdue.invoice_number || 'Invoice'}`,
                meta: 'Perlu tindak lanjut',
                link: '/admin/billing/invoices'
            });
        }

        const recentPaidInvoices = recentInvoices
            .filter(invoice => String(invoice.status || '').trim().toLowerCase() === 'paid')
            .filter(invoice => {
                const paidAtRaw = invoice.paid_at || invoice.payment_date || invoice.updated_at || invoice.created_at;
                if (!paidAtRaw) return false;
                const paidAt = new Date(paidAtRaw).getTime();
                return Number.isFinite(paidAt) && (now - paidAt) <= (24 * 60 * 60 * 1000);
            })
            .sort((a, b) => {
                const aTime = new Date(a.paid_at || a.payment_date || a.updated_at || a.created_at || 0).getTime();
                const bTime = new Date(b.paid_at || b.payment_date || b.updated_at || b.created_at || 0).getTime();
                return bTime - aTime;
            });

        if (recentPaidInvoices.length > 0) {
            const latestPaid = recentPaidInvoices[0];
            billingNotifications.push({
                id: 'recent-paid',
                type: 'success',
                icon: 'bi-check-circle-fill',
                title: 'Pembayaran Baru Lunas',
                message: `${recentPaidInvoices.length} tagihan baru lunas hari ini. Terbaru: ${latestPaid.customer_name || 'Pelanggan'} - ${latestPaid.invoice_number || 'Invoice'}`,
                meta: 'Status paid',
                link: '/admin/billing/payments'
            });
        }
...
router.get('/api/revenue/summary', adminAuth, async (req, res) => {
    try {
        ...
        const dbPath = path.join(__dirname, '../data/billing.db');
        const db = new sqlite3.Database(dbPath);
        const billingStats = await billingManager.getBillingStats();
        ...
        // Sinkronkan dengan angka Dashboard Billing (invoice paid bulan ini)
        const monthRevenue = (billingStats.current_monthly_revenue || 0) + (billingStats.current_voucher_revenue || 0);
...
router.get('/invoices-by-type', adminAuth, async (req, res) => {
    try {
        const now = new Date();
        const selectedMonth = Math.min(12, Math.max(1, parseInt(req.query.month, 10) || (now.getMonth() + 1)));
        const selectedYear = parseInt(req.query.year, 10) || now.getFullYear();

        const getInvoiceDateParts = (invoice) => {
            const source = String(invoice.created_at || invoice.payment_date || invoice.due_date || '').trim();
        ...
        const matchesSelectedPeriod = (invoice) => {
            const parts = getInvoiceDateParts(invoice);
            if (!parts) return false;
            return parts.year === selectedYear && parts.month === selectedMonth;
        };

        const normalizeInvoices = (invoices = []) => {
            const filtered = invoices.filter(matchesSelectedPeriod);
            return filtered.sort((a, b) => new Date(b.created_at || b.due_date || 0) - new Date(a.created_at || a.due_date || 0));
        };

        const buildStats = (invoices = []) => {
            const paidInvoices = invoices.filter((invoice) => String(invoice.status || '').trim().toLowerCase() === 'paid');
            const unpaidInvoices = invoices.filter((invoice) => String(invoice.status || '').trim().toLowerCase() !== 'paid');

            return {
                total_invoices: invoices.length,
                paid_invoices: paidInvoices.length,
                unpaid_invoices: unpaidInvoices.length,
                total_revenue: paidInvoices.reduce((sum, invoice) => sum + (parseFloat(invoice.amount) || 0), 0),
                total_unpaid: unpaidInvoices.reduce((sum, invoice) => sum + (parseFloat(invoice.amount) || 0), 0)
            };
        };

        const [allMonthlyInvoices, allVoucherInvoices, allManualInvoices] = await Promise.all([
            billingManager.getInvoicesByType('monthly'),
            billingManager.getInvoicesByType('voucher'),
            billingManager.getInvoicesByType('manual')
        ]);

        const monthlyInvoices = normalizeInvoices(allMonthlyInvoices);
        const voucherInvoices = normalizeInvoices(allVoucherInvoices);
        const manualInvoices = normalizeInvoices(allManualInvoices);

        const monthlyStats = buildStats(monthlyInvoices);
        const voucherStats = buildStats(voucherInvoices);
        const manualStats = buildStats(manualInvoices);
        ...
        const allInvoices = [...allMonthlyInvoices, ...allVoucherInvoices, ...allManualInvoices];
        const yearSet = new Set(
            allInvoices
                .map(getInvoiceDateParts)
                .filter(Boolean)
        ...
        res.render('admin/billing/invoices-by-type', {
            ...
            page: 'invoices',
            subPage: 'invoices-by-type',
            selectedYear,
            selectedMonth,
            availableYears,
            monthOptions,
            monthlyInvoices: monthlyInvoices.slice(0, 100),
            voucherInvoices: voucherInvoices.slice(0, 100),
            manualInvoices: manualInvoices.slice(0, 100),
            monthlyStats,
            voucherStats,
            manualStats,
            appSettings: req.appSettings
...
router.post('/customers/:phone/house-photo', uploadCustomerHousePhoto.single('house_photo'), async (req, res) => {
    try {
        const { phone } = req.params;
        if (!req.file) {
            return res.status(400).json({ success: false, message: 'File foto rumah tidak ditemukan' });
        }

        const db = require('../config/billing').db;
        db.run('UPDATE customers SET house_photo = ? WHERE phone = ?', [req.file.filename, phone], function (err) {
            if (err) {
                logger.error('Error saving customer house photo:', err);
                return res.status(500).json({ success: false, message: 'Gagal menyimpan foto rumah' });
            }
            if (this.changes === 0) {
            ...
            return res.json({
                success: true,
                message: 'Foto rumah berhasil diupload',
                filename: req.file.filename,
                path: `/img/customer-houses/${req.file.filename}`
            });
        });
    } catch (error) {
        logger.error('Error uploading customer house photo:', error);
        res.status(500).json({ success: false, message: 'Terjadi kesalahan saat upload foto rumah' });
    }
});
...
router.post('/customers/:phone/profile-photo', uploadCustomerProfilePhoto.single('profile_photo'), async (req, res) => {
    try {
        const { phone } = req.params;
        if (!req.file) {
            return res.status(400).json({ success: false, message: 'File foto profil tidak ditemukan' });
        }

        const db = require('../config/billing').db;
        db.run('UPDATE customers SET profile_photo = ? WHERE phone = ?', [req.file.filename, phone], function (err) {
            if (err) {
                logger.error('Error saving customer profile photo:', err);
            ...
            return res.json({
                ...
                filename: req.file.filename,
                path: `/img/customer-profiles/${req.file.filename}`
            });
        });
    } catch (error) {
        logger.error('Error uploading customer profile photo:', error);
        res.status(500).json({ success: false, message: 'Terjadi kesalahan saat upload foto profil' });
    }
});

router.post('/payment-transactions/:id/resolve-pending', async (req, res) => {
    try {
        ...
        const adminLabel = req.session?.admin?.username || req.session?.admin?.name || 'admin';
        const result = await billingManager.resolvePendingPaymentTransaction(transactionId, adminLabel);

        return res.json({
            success: true,
            message: 'Pending berhasil diselesaikan dan invoice ditandai lunas',
            data: result
        });
    } catch (error) {
        logger.error('Error resolving pending payment transaction:', error);
        return res.status(500).json({
            success: false,
            message: error.message || 'Gagal menyelesaikan pending'
        });
    }
});

router.post('/payment-transactions/:id/cancel-pending', async (req, res) => {
    try {
        const transactionId = parseInt(req.params.id, 10);
        if (!transactionId) {
            return res.status(400).json({ success: false, message: 'ID transaksi tidak valid' });
        ...
        const adminLabel = req.session?.admin?.username || req.session?.admin?.name || 'admin';
        const result = await billingManager.cancelPendingPaymentTransaction(transactionId, adminLabel);

        return res.json({
            success: true,
            message: 'Pending berhasil dibatalkan',
            data: result
        });
    } catch (error) {
        logger.error('Error cancelling pending payment transaction:', error);
        return res.status(500).json({
            success: false,
            message: error.message || 'Gagal membatalkan pending'
        });
    }
});
...
router.get('/mapping-new', getAppSettings, async (req, res) => {
    try {
        return res.redirect('/admin/mapping-new');
    } catch (error) {
...
router.get('/reports', getAppSettings, async (req, res) => {
    try {
        ...
        // Sinkronkan dengan Dashboard Billing (bulan berjalan)
        const billingStats = await billingManager.getBillingStats();
        const totalCustomers = billingStats.total_customers || 0;
        const totalInvoices = (billingStats.current_monthly_invoices_total || 0) + (billingStats.current_voucher_invoices_total || 0);
        const totalRevenue = (billingStats.current_monthly_revenue || 0) + (billingStats.current_voucher_revenue || 0);
        const pendingPayments = await billingManager.getActivePendingGatewayPayments();
        const paidInvoices = (billingStats.current_paid_monthly_invoices || 0) + (billingStats.current_paid_voucher_invoices || 0);
        const paymentSuccess = paidInvoices; // termasuk manual_admin selama invoice status=paid
