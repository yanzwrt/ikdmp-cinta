...
const adminProfileStorage = multer.diskStorage({
    destination: function (req, file, cb) {
        const uploadPath = path.join(__dirname, '../public/img/admin-profiles');
...
router.post('/save', (req, res) => {
    try {
        ...
        // Paksa timezone sistem selalu Asia/Jakarta (WIB)
        mergedSettings.timezone = 'Asia/Jakarta';
...
router.post('/upload-admin-profile', uploadAdminProfile.single('admin_profile'), (req, res) => {
    try {
        if (!req.file) {
            return res.status(400).json({
                success: false,
        ...
        const adminProfileDir = path.join(__dirname, '../public/img/admin-profiles');
        ...
        const settings = getSettingsWithCache();
        if (settings.admin_profile_photo && settings.admin_profile_photo !== filename) {
            const oldPath = path.join(adminProfileDir, settings.admin_profile_photo);
            if (fs.existsSync(oldPath)) {
                try {
                    fs.unlinkSync(oldPath);
                } catch (unlinkError) {
                    logger.warn(`Gagal menghapus foto profil admin lama: ${unlinkError.message}`);
                }
            }
        }
        settings.admin_profile_photo = filename;
        ...
        res.json({
            success: true,
            filename,
            path: `/img/admin-profiles/${filename}`,
            message: 'Foto profil admin berhasil diupload'
        });
    } catch (error) {
        console.error('Error saat upload foto profil admin:', error);
        res.status(500).json({
            success: false,
            error: 'Terjadi kesalahan saat mengupload foto profil admin'
        });
    }
});
...
router.post('/telegram/test', async (req, res) => {
    try {
        ...
        await telegramBot.bot.sendMessage(chatId, '✅ Ini adalah pesan test dari sistem IKDMP-CINTA.');
