...
class AgentManager {
    ...
    createTables() {
        ...
        // Create indexes
        this.createIndexes();
        this.ensureAgentApprovalColumns();
        this.ensureAgentProfilePhotoColumn();
    }
    ...
    ensureAgentApprovalColumns() {
        const requiredColumns = [
            {
                name: 'approval_status',
                sql: `ALTER TABLE agents ADD COLUMN approval_status TEXT DEFAULT 'approved' CHECK (approval_status IN ('pending', 'approved', 'rejected'))`
            },
            { name: 'approval_notes', sql: `ALTER TABLE agents ADD COLUMN approval_notes TEXT` },
            { name: 'approved_at', sql: `ALTER TABLE agents ADD COLUMN approved_at DATETIME` },
            { name: 'approved_by', sql: `ALTER TABLE agents ADD COLUMN approved_by TEXT` },
            { name: 'rejected_at', sql: `ALTER TABLE agents ADD COLUMN rejected_at DATETIME` },
            { name: 'rejected_by', sql: `ALTER TABLE agents ADD COLUMN rejected_by TEXT` }
        ];

        const normalizeApprovalStatus = () => {
            this.db.all(`PRAGMA table_info(agents)`, [], (checkErr, latestColumns) => {
                if (checkErr) {
                    logger.error('Error re-checking agent columns:', checkErr);
                    return;
                }

                const latestColumnSet = new Set((latestColumns || []).map((column) => column.name));
                if (!latestColumnSet.has('approval_status')) {
                    logger.warn('Skipping approval status normalization because approval_status column is still unavailable');
                    return;
                }
                ...
                this.db.run(
                    `UPDATE agents
                     SET approval_status = CASE
                            WHEN approval_status IS NULL AND status = 'active' THEN 'approved'
                            WHEN approval_status IS NULL AND status = 'inactive' THEN 'pending'
                    ...
                    (updateErr) => {
                        if (updateErr) {
                            logger.error('Error normalizing existing agent approval statuses:', updateErr);
                        }
                    }
                );
            });
        ...
        this.db.all(`PRAGMA table_info(agents)`, [], (err, columns) => {
            if (err) {
                logger.error('Error checking agent columns:', err);
                return;
            }
            ...
            missingColumns.forEach((column) => {
                this.db.run(column.sql, (alterErr) => {
                    if (alterErr) {
                        const message = String(alterErr.message || '');
                        if (!message.includes('duplicate column name')) {
                            logger.error('Error altering agents table for approval flow:', alterErr);
                        }
                    }
    ...
    ensureAgentProfilePhotoColumn() {
        this.db.all(`PRAGMA table_info(agents)`, [], (err, columns) => {
            if (err) {
                logger.error('Error checking agent profile_photo column:', err);
                return;
            }
            ...
            this.db.run(`ALTER TABLE agents ADD COLUMN profile_photo TEXT`, (alterErr) => {
                if (alterErr) {
                    const message = String(alterErr.message || '');
                    if (!message.includes('duplicate column name')) {
                        logger.error('Error adding profile_photo to agents table:', alterErr);
                    }
                }
            });
        });
    ...
    async authenticateAgent(username, password) {
        return new Promise((resolve, reject) => {
            const sql = 'SELECT * FROM agents WHERE username = ? OR phone = ?';
            this.db.get(sql, [username, username], async (err, agent) => {
                ...
                try {
                    ...
                    if (agent.approval_status === 'pending') {
                        resolve({ success: false, message: 'Pendaftaran Anda masih menunggu persetujuan admin' });
                        return;
                    }

                    if (agent.approval_status === 'rejected') {
                        const rejectReason = agent.approval_notes ? ` Alasan: ${agent.approval_notes}` : '';
                        resolve({ success: false, message: `Pendaftaran Anda ditolak.${rejectReason}` });
                        return;
                    }

                    if (agent.status !== 'active') {
                        resolve({ success: false, message: 'Akun agent tidak aktif' });
                        return;
                    }
    ...
    async createAgent(agentData) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                const sql = `INSERT INTO agents (username, name, phone, email, password, address, commission_rate, status, approval_status, profile_photo) 
                           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
                const db = this.db;
                db.run(sql, [
                    agentData.username,
                    ...
                    db.run(insertVoucherSql, [
                        ...
                        db.run(updateBalanceSql, [packageData.agentPrice, agentId], function(err) {
                            ...
                            db.run(insertTransactionSql, [
                                ...
                                `Penjualan voucher ${packageData.name}${customerName ? ` (Customer: ${customerName})` : ''}${customerPhone ? ` (${customerPhone})` : ''}`,
                                voucherCode
                            ], function(err) {
                                if (err) {
                                    db.run('ROLLBACK');
    ...
    async getAgentByPhone(phone) {
        ...
        return new Promise((resolve, reject) => {
            const sql = 'SELECT * FROM agents WHERE (phone = ? OR phone = ?) AND status = "active" AND COALESCE(approval_status, \'approved\') = "approved"';
    ...
    async getAnyAgentByPhone(phone) {
        let normalized = String(phone || '').replace(/\D/g, '');
        if (normalized.startsWith('0')) normalized = '62' + normalized.slice(1);
        if (normalized && !normalized.startsWith('62')) normalized = '62' + normalized;
        const local = normalized.replace(/^62/, '0');

        return new Promise((resolve, reject) => {
            const sql = 'SELECT * FROM agents WHERE phone = ? OR phone = ?';
            this.db.get(sql, [normalized, local], (err, row) => {
                if (err) {
                    reject(err);
                    return;
    ...
    async getAgentRegistrationByIdentifier(identifier) {
        const raw = String(identifier || '').trim();
        const digits = raw.replace(/\D/g, '');
        const normalized = digits
            ? (digits.startsWith('0') ? `62${digits.slice(1)}` : (digits.startsWith('62') ? digits : `62${digits}`))
            : null;
        const local = normalized ? normalized.replace(/^62/, '0') : null;
        ...
        return new Promise((resolve, reject) => {
            const sql = `
                ...
                LIMIT 1
            `;
            this.db.get(sql, [parseInt(raw, 10) || 0, raw, raw, normalized || '', local || ''], (err, row) => {
                if (err) {
                    reject(err);
                    return;
                }
    ...
    // Update agent status
    async updateAgentStatus(agentId, status, extraData = {}) {
        return new Promise((resolve, reject) => {
            const approvalStatus = extraData.approval_status;
            const approvalNotes = extraData.approval_notes || null;
            const approvedBy = extraData.approved_by || null;
            const rejectedBy = extraData.rejected_by || null;
            const sql = `
                UPDATE agents
                SET status = ?,
    ...
    async updateAgent(agentId, agentData) {
        return new Promise((resolve, reject) => {
            ...
            if (Object.prototype.hasOwnProperty.call(agentData, 'profile_photo')) {
                sql += ', profile_photo = ?';
                params.push(agentData.profile_photo || null);
            }
