...
class IntervalManager {
    ...
    startOfflineNotification() {
        try {
            ...
            // Ambil interval dari settings (dalam milliseconds)
            // Default dipercepat menjadi 15 menit (900000 ms) agar notifikasi lebih real-time
            const intervalMs = parseInt(getSetting('offline_notification_interval', 900000)); 
            const intervalMinutes = Math.round(intervalMs / (1000 * 60));
            
            logger.info(`Starting Offline Notification monitoring (interval: ${intervalMinutes} minutes)`);
            ...
            this.timeouts.offlineNotification = setTimeout(() => {
                // Gunakan threshold rendah (0.2 jam = 12 menit) agar cepat terdeteksi
                monitorOfflineDevices(0.2).catch(err => {
                    logger.error('Error in initial Offline Notification check:', err.message);
                });
            }, 2 * 60 * 1000); // Delay 2 menit

            // Set interval untuk pengecekan berkala
            this.intervals.offlineNotification = setInterval(() => {
                // Gunakan threshold rendah (0.2 jam = 12 menit)
                monitorOfflineDevices(0.2).catch(err => {
