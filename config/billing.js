...
class BillingManager {
                            ...
                            try {
                                ...
                                if (existingRoute) {
                                    ...
                                    db.run(updateSql, [
                                        ...
                                        if (err) {
                                            console.error(`âŒ Error updating cable route for customer ${oldCustomer.username}:`, err.message);
                                        } else {
    ...
    createTables() {
        ...
        // Tambahkan kolom house_photo ke customers jika belum ada
        this.db.run("ALTER TABLE customers ADD COLUMN house_photo TEXT", (err) => {
            if (err && !err.message.includes('duplicate column name')) {
                console.error('Error adding house_photo column to customers:', err);
            } else if (!err) {
                console.log('Added house_photo column to customers table');
            }
        });

        this.db.run("ALTER TABLE customers ADD COLUMN requested_ssid TEXT", (err) => {
            if (err && !err.message.includes('duplicate column name')) {
                console.error('Error adding requested_ssid column to customers:', err);
            } else if (!err) {
                console.log('Added requested_ssid column to customers table');
            }
        });

        this.db.run("ALTER TABLE customers ADD COLUMN requested_wifi_password TEXT", (err) => {
            if (err && !err.message.includes('duplicate column name')) {
                console.error('Error adding requested_wifi_password column to customers:', err);
            } else if (!err) {
                console.log('Added requested_wifi_password column to customers table');
            }
        });

        // Tambahkan kolom photo ke odps jika belum ada
        this.db.run("ALTER TABLE odps ADD COLUMN photo TEXT", (err) => {
            if (err && !err.message.includes('duplicate column name')) {
                console.error('Error adding photo column to odps:', err);
            } else if (!err) {
                console.log('Added photo column to odps table');
            }
        });
    ...
    addColumnsAndCreateIndexes() {
        ...
        this.db.run("ALTER TABLE customers ADD COLUMN profile_photo TEXT", (err) => {
            if (err && !err.message.includes('duplicate column name')) {
                console.error('Error adding profile_photo column:', err);
            } else if (!err) {
                console.log('Added profile_photo column to customers table');
            }
        });
        ...
        this.db.run("ALTER TABLE customers ADD COLUMN password TEXT", (err) => {
            if (err && !err.message.includes('duplicate column name')) {
                ...
                console.log('âœ… Added password column to customers table');
            }
        });
    ...
    async createCustomer(customerData) {
        return new Promise(async (resolve, reject) => {
            ...
            if (!this.db) {
                console.error('âŒ Database not initialized');
            ...
            const { name, username, phone, pppoe_username, email, address, package_id, odp_id, pppoe_profile, status, auto_suspension, billing_day, static_ip, assigned_ip, mac_address, latitude, longitude, cable_type, cable_length, port_number, cable_status, cable_notes, profile_photo, house_photo, requested_ssid, requested_wifi_password } = customerData;
            ...
            const sql = `INSERT INTO customers (username, name, phone, pppoe_username, email, address, package_id, odp_id, pppoe_profile, status, auto_suspension, billing_day, static_ip, assigned_ip, mac_address, latitude, longitude, cable_type, cable_length, port_number, cable_status, cable_notes, profile_photo, house_photo, requested_ssid, requested_wifi_password) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
                ...
                if (err) {
                    ...
                    if (odp_id) {
                        ...
                        try {
                            ...
                            db.run(cableRouteSql, [
                                ...
                                if (err) {
                                    console.error(`âŒ Error creating cable route for customer ${finalUsername}:`, err.message);
                                } else {
                                    console.log(`âœ… Successfully created cable route for customer ${finalUsername}`);
                                }
                            });
                        } catch (cableError) {
                            console.error(`âŒ Error creating cable route for customer ${finalUsername}:`, cableError.message);
                    ...
                    if (phone && autoPPPoEUsername) {
                        try {
                            ...
                            console.log(`âš ï¸ GenieACS integration skipped for customer ${finalUsername} (timeout or error): ${genieacsError.message}`);
                            // Jangan reject, karena customer sudah berhasil dibuat di billing
                        }
    ...
    async updateCustomerWifiInfoByPhone(phone, wifiData = {}) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (wifiData.password !== undefined) {
                    fields.push('password = ?');
                    params.push(wifiData.password);
                }

                if (wifiData.requested_ssid !== undefined) {
                    fields.push('requested_ssid = ?');
                    params.push(wifiData.requested_ssid);
                }

                if (wifiData.requested_wifi_password !== undefined) {
                    fields.push('requested_wifi_password = ?');
                    params.push(wifiData.requested_wifi_password);
                }
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                this.db.run(deleteCableRoutesSql, [customer.id], function (err) {
                    if (err) {
                        console.error(`âŒ Error deleting cable routes for customer ${customer.username}:`, err.message);
                    } else {
        ...
        return new Promise((resolve, reject) => {
            ...
            const sql = `
                SELECT 
                    ...
                    (SELECT COUNT(*) FROM invoices WHERE invoice_type = 'monthly' AND status = 'paid' AND strftime('%Y-%m', payment_date) = strftime('%Y-%m', 'now')) as current_paid_monthly_invoices,
                    (SELECT COUNT(*) FROM invoices WHERE invoice_type = 'voucher' AND status = 'paid' AND strftime('%Y-%m', payment_date) = strftime('%Y-%m', 'now')) as current_paid_voucher_invoices,
                    (SELECT COALESCE(SUM(amount), 0) FROM invoices WHERE invoice_type = 'monthly' AND status = 'paid' AND strftime('%Y-%m', payment_date) = strftime('%Y-%m', 'now')) as current_monthly_revenue,
                    (SELECT COALESCE(SUM(amount), 0) FROM invoices WHERE invoice_type = 'voucher' AND status = 'paid' AND strftime('%Y-%m', payment_date) = strftime('%Y-%m', 'now')) as current_voucher_revenue,
                    (SELECT COUNT(*) FROM invoices WHERE invoice_type = 'monthly' AND strftime('%Y-%m', created_at) = strftime('%Y-%m', 'now')) as current_monthly_invoices_total,
                    (SELECT COUNT(*) FROM invoices WHERE invoice_type = 'monthly' AND status = 'paid' AND strftime('%Y-%m', created_at) = strftime('%Y-%m', 'now')) as current_monthly_invoices_paid,
                    (SELECT COUNT(*) FROM invoices WHERE invoice_type = 'monthly' AND status = 'unpaid' AND strftime('%Y-%m', created_at) = strftime('%Y-%m', 'now')) as current_monthly_invoices_unpaid,
                    (SELECT COUNT(*) FROM invoices WHERE invoice_type = 'voucher' AND strftime('%Y-%m', created_at) = strftime('%Y-%m', 'now')) as current_voucher_invoices_total,
                    (SELECT COUNT(*) FROM invoices WHERE invoice_type = 'voucher' AND status = 'paid' AND strftime('%Y-%m', created_at) = strftime('%Y-%m', 'now')) as current_voucher_invoices_paid,
                    (SELECT COUNT(*) FROM invoices WHERE invoice_type = 'voucher' AND status = 'unpaid' AND strftime('%Y-%m', created_at) = strftime('%Y-%m', 'now')) as current_voucher_invoices_unpaid,
                    (SELECT COALESCE(SUM(amount), 0) FROM invoices WHERE invoice_type = 'monthly' AND status = 'unpaid' AND strftime('%Y-%m', created_at) = strftime('%Y-%m', 'now')) as current_monthly_unpaid,
                    (SELECT COALESCE(SUM(amount), 0) FROM invoices WHERE invoice_type = 'voucher' AND status = 'unpaid' AND strftime('%Y-%m', created_at) = strftime('%Y-%m', 'now')) as current_voucher_unpaid,
            ...
            this.db.get(sql, [], (err, row) => {
                if (err) {
                    ...
                    const stats = {
                        ...
                        current_paid_monthly_invoices: parseInt(row.current_paid_monthly_invoices) || 0,
                        current_paid_voucher_invoices: parseInt(row.current_paid_voucher_invoices) || 0,
                        ...
                        current_monthly_revenue: parseFloat(row.current_monthly_revenue) || 0,
                        current_voucher_revenue: parseFloat(row.current_voucher_revenue) || 0,

                        // Current month invoices by type
                        current_monthly_invoices_total: parseInt(row.current_monthly_invoices_total) || 0,
                        current_monthly_invoices_paid: parseInt(row.current_monthly_invoices_paid) || 0,
                        current_monthly_invoices_unpaid: parseInt(row.current_monthly_invoices_unpaid) || 0,
                        current_voucher_invoices_total: parseInt(row.current_voucher_invoices_total) || 0,
                        current_voucher_invoices_paid: parseInt(row.current_voucher_invoices_paid) || 0,
                        current_voucher_invoices_unpaid: parseInt(row.current_voucher_invoices_unpaid) || 0,
                        current_monthly_unpaid: parseFloat(row.current_monthly_unpaid) || 0,
                        current_voucher_unpaid: parseFloat(row.current_voucher_unpaid) || 0,
    ...
    async saveMonthlySummary(year, month, stats, notes = null) {
        return new Promise((resolve, reject) => {
            ...
            // Use current_ fields if available (from getBillingStats), otherwise fallback to passed values
            // Note: We prioritize current_ fields because stats.monthly_invoices usually contains lifetime counts
            
            const currentMonthlyInvoices = (stats.current_monthly_invoices_total !== undefined) ? stats.current_monthly_invoices_total : (stats.monthly_invoices || 0);
            const currentVoucherInvoices = (stats.current_voucher_invoices_total !== undefined) ? stats.current_voucher_invoices_total : (stats.voucher_invoices || 0);
            
            const currentPaidMonthly = (stats.current_monthly_invoices_paid !== undefined) ? stats.current_monthly_invoices_paid : (stats.paid_monthly_invoices || 0);
            const currentPaidVoucher = (stats.current_voucher_invoices_paid !== undefined) ? stats.current_voucher_invoices_paid : (stats.paid_voucher_invoices || 0);
            
            const currentUnpaidMonthly = (stats.current_monthly_invoices_unpaid !== undefined) ? stats.current_monthly_invoices_unpaid : (stats.unpaid_monthly_invoices || 0);
            const currentUnpaidVoucher = (stats.current_voucher_invoices_unpaid !== undefined) ? stats.current_voucher_invoices_unpaid : (stats.unpaid_voucher_invoices || 0);
            
            const currentMonthlyRevenue = (stats.current_monthly_revenue !== undefined) ? stats.current_monthly_revenue : (stats.monthly_revenue || 0);
            const currentVoucherRevenue = (stats.current_voucher_revenue !== undefined) ? stats.current_voucher_revenue : (stats.voucher_revenue || 0);
            
            const currentMonthlyUnpaid = (stats.current_monthly_unpaid !== undefined) ? stats.current_monthly_unpaid : (stats.monthly_unpaid || 0);
            const currentVoucherUnpaid = (stats.current_voucher_unpaid !== undefined) ? stats.current_voucher_unpaid : (stats.voucher_unpaid || 0);

            const params = [
                year, month,
                stats.total_customers || 0,
                stats.active_customers || 0,
                currentMonthlyInvoices,
                currentVoucherInvoices,
                currentPaidMonthly,
                currentPaidVoucher,
                currentUnpaidMonthly,
                currentUnpaidVoucher,
                currentMonthlyRevenue,
                currentVoucherRevenue,
                currentMonthlyUnpaid,
                currentVoucherUnpaid,
                (currentMonthlyRevenue + currentVoucherRevenue), // Total Revenue
                (currentMonthlyUnpaid + currentVoucherUnpaid),   // Total Unpaid
                notes
            ];
    ...
    async performMonthlyReset() {
        try {
            ...
            logger.info(`ðŸ”„ Starting monthly reset for ${currentYear}-${currentMonth}`);
            ...
            logger.info(`âœ… Admin monthly summary saved for ${prevYear}-${prevMonth}`);
            ...
            for (const collector of collectors) {
                ...
                logger.info(`âœ… Collector ${collector.name} monthly summary saved for ${prevYear}-${prevMonth}`);
            }

            // 3. Create collector_monthly_summary table if not exists
            await this.ensureCollectorMonthlySummaryTable();

            logger.info(`ðŸŽ‰ Monthly reset completed successfully for ${currentYear}-${currentMonth}`);
            ...
            try {
                ...
                if (type === 'income') {
                    ...
                    sql = `
                        ...
                        AND (p.payment_type IN ('direct', 'collector', 'online', 'manual') OR p.payment_type IS NULL)
                ...
                this.db.all(sql, params, (err, rows) => {
                    if (err) {
                        ...
                        // Dedupe pemasukan berdasarkan invoice agar tidak double count
                        // (contoh: transaksi gateway + payments untuk invoice yang sama).
                        ...
                        for (const r of incomeRows) {
                            const invoiceKey = (r.invoice_number || '').toString().trim();
                            const fallbackKey = [
                                r.date || '',
                                r.amount || 0,
                                r.customer_phone || '',
                            ...
                            const key = invoiceKey ? `inv:${invoiceKey}` : `fallback:${fallbackKey}`;
                            if (seenIncomeKeys.has(key)) continue;
                            seenIncomeKeys.add(key);
                            dedupedIncomeRows.push(r);
                        }

                        const mergedRows = dedupedIncomeRows.concat(expenseRows).sort((a, b) => {
                            const da = new Date(a.date || 0).getTime();
                            const db = new Date(b.date || 0).getTime();
                            return db - da;
                        });

                        // Hitung total dan statistik dari data yang sudah dedupe
                        const totalIncome = dedupedIncomeRows
                            .reduce((sum, r) => sum + (r.amount || 0), 0);
                        const totalExpense = expenseRows
                            .reduce((sum, r) => sum + (r.amount || 0), 0);
                        const totalCommission = dedupedIncomeRows
                            .reduce((sum, r) => sum + (r.commission_amount || 0), 0);
                        const netProfit = totalIncome - totalExpense;

                        // Statistik per tipe pembayaran
                        const incomeByType = dedupedIncomeRows
                            .reduce((acc, r) => {
                        ...
                        const result = {
                            ...
                            summary: {
                                ...
                                incomeCount: dedupedIncomeRows.length,
    ...
    async getPaymentTransactionById(transactionId) {
        return new Promise((resolve, reject) => {
            const sql = `
                SELECT pgt.*, i.invoice_number, i.status AS invoice_status, i.payment_status AS invoice_payment_status,
    ...
    async getActivePendingGatewayPayments() {
        return new Promise((resolve, reject) => {
            const sql = `
                SELECT COUNT(DISTINCT pgt.invoice_id) as count
                FROM payment_gateway_transactions pgt
                INNER JOIN invoices i ON i.id = pgt.invoice_id
                WHERE pgt.status = 'pending'
                  AND i.status = 'unpaid'
            `;
            this.db.get(sql, [], (err, row) => {
                if (err) {
    ...
    async getActivePendingGatewayTransactions() {
        return new Promise((resolve, reject) => {
            const sql = `
                SELECT
                    pgt.*,
    ...
    async resolvePendingPaymentTransaction(transactionId, adminLabel = 'Admin') {
        const transaction = await this.getPaymentTransactionById(transactionId);
        if (!transaction) {
            throw new Error('Transaksi payment gateway tidak ditemukan');
        }

        if (transaction.status === 'success' || transaction.status === 'settlement' || transaction.status === 'PAID') {
            throw new Error('Transaksi ini sudah diselesaikan sebelumnya');
        }

        if (transaction.invoice_status === 'paid') {
            throw new Error('Invoice ini sudah berstatus lunas');
        }
            ...
            this.db.run(
                `UPDATE payment_gateway_transactions
                 SET status = 'resolved_manual',
                     payment_type = COALESCE(payment_type, 'manual_admin'),
                     fraud_status = ?,
                     updated_at = CURRENT_TIMESTAMP
                 WHERE id = ?`,
                [`resolved_by_${adminLabel}`.replace(/\s+/g, '_').toLowerCase(), transactionId],
                function (err) {
                    if (err) return reject(err);
                    resolve();
                }
        ...
        const existingPayment = await new Promise((resolve, reject) => {
            this.db.get(
                `SELECT id FROM payments WHERE invoice_id = ? AND reference_number = ? LIMIT 1`,
                [transaction.invoice_id, transaction.order_id],
        ...
        if (!existingPayment) {
            const paymentResult = await this.recordPayment({
                ...
                notes: `Pending gateway ${transaction.gateway} diselesaikan manual oleh ${adminLabel}`
            });
            paymentId = paymentResult.id;
        }
        ...
        const invoice = await this.getInvoiceById(transaction.invoice_id);
        const customer = await this.getCustomerById(transaction.customer_id);

        if (customer && invoice) {
            try {
                await this.sendPaymentSuccessNotification(customer, invoice);
            } catch (notificationError) {
                logger.error('[PENDING] Failed sending payment success notification after resolving pending:', notificationError);
            }

            try {
                const refreshedCustomer = await this.getCustomerById(transaction.customer_id);
                const invoices = await this.getInvoicesByCustomer(transaction.customer_id);
                const unpaid = invoices.filter(i => i.status === 'unpaid');
                if (refreshedCustomer && refreshedCustomer.status === 'suspended' && unpaid.length === 0) {
                    const serviceSuspension = require('./serviceSuspension');
                    await serviceSuspension.restoreCustomerService(refreshedCustomer, 'Pending payment resolved by admin');
                }
            } catch (restoreErr) {
                logger.error('[PENDING] Failed restore after resolving pending payment:', restoreErr);
            }
        }
    ...
    async cancelPendingPaymentTransaction(transactionId, adminLabel = 'Admin') {
        const transaction = await this.getPaymentTransactionById(transactionId);
        if (!transaction) {
            throw new Error('Transaksi payment gateway tidak ditemukan');
        }
        ...
        if (transaction.status !== 'pending') {
            throw new Error(`Transaksi ini sudah berstatus ${transaction.status}`);
        }
            ...
            this.db.run(
                `UPDATE payment_gateway_transactions
                 SET status = 'cancelled',
                     ...
                     updated_at = CURRENT_TIMESTAMP
                 WHERE id = ?`,
                [`cancelled_by_${adminLabel}`.replace(/\s+/g, '_').toLowerCase(), transactionId],
                function (err) {
                    if (err) return reject(err);
                    resolve();
                }
        ...
        const stillPending = await new Promise((resolve, reject) => {
            this.db.get(
                `SELECT COUNT(*) AS total
                 FROM payment_gateway_transactions
                 WHERE invoice_id = ? AND status = 'pending'`,
                [transaction.invoice_id],
                (err, row) => err ? reject(err) : resolve(parseInt(row?.total, 10) || 0)
            );
        });
...
*${getCompanyHeader()}*
Info: ${getSetting('contact_whatsapp', '082130077713')}`;
