        ...
        monitorInterval = setInterval(async () => {
            try {
                ...
                if (loginUsers.length > 0) {
                    // Suppress login notification as per user request
                    /*
                    ...
                    */
                    logger.info('PPPoE LOGIN detected (Notification suppressed):', loginUsers);
                }
                ...
                if (logoutUsers.length > 0) {
                    ...
                    // Kirim ke group WhatsApp (JIKA DIKONFIGURASI)
                    const techGroupId = getSetting('technician_group_id');
                    if (sock && techGroupId) {
                        try {
                            await sock.sendMessage(techGroupId, { text: msg });
                        } catch (e) {
                            logger.error('Gagal kirim notifikasi PPPoE LOGOUT ke WhatsApp group:', e);
                        }
                    }
                    // Jika tidak ada group ID, kita tidak log warning lagi agar tidak spam
